...
public class Rota3DSwitchView extends FrameLayout {
    Camera mCamera;
    Matrix mMaxtrix;

    public Rota3DSwitchView(Context context) {
        super(context);
        initRoat3D();
    }
    ...
    public Rota3DSwitchView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initRoat3D();
    }

    public Rota3DSwitchView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initRoat3D();
    }

//    public Rota3DSwitchView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
...
//    }
    ...
    protected void dispatchDraw(Canvas canvas) {
        int centerX = getWidth() / 2;//中间显示视图 ----左边的位置
        ...
        for (int i = 0; i < 4; i++)
            chilDrawforCamera(canvas, postTranslateX, centerX, i);
//        if (!isTouch)
...
//    };
    ...
    private void chilDrawforCamera(Canvas canvas, int postTranslateX, int centerX, int i) {
        canvas.save();
        mCamera.save();
        mMaxtrix.reset();
//        if (i == 1) {
...
//        }
        ...
        mCamera.restore();
        mMaxtrix.preTranslate(-centerX, -getHeight() / 2);//指定在 屏幕上 运行的棱 是哪一条
        mMaxtrix.postTranslate(centerX, getHeight() / 2);//运行路径
