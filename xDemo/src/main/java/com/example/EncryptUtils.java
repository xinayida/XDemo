...
public final class EncryptUtils {
    ...
    public static byte[] aesEncrypt(byte[] srcBytes, String aesKey){
        try {
            Cipher cipher = Cipher.getInstance(AES_CBC_PCK_ALG,"BC");

            IvParameterSpec iv = new IvParameterSpec(AES_IV);
            cipher.init(Cipher.ENCRYPT_MODE, convertKey(aesKey)
                    /*new SecretKeySpec(base64Decode(aesKey.getBytes()), AES_ALG)*/, iv);

            return cipher.doFinal(srcBytes);
        } catch (Exception e) {
            e.printStackTrace();
            throw new IllegalArgumentException( "AES加密失败 " +  e.toString());
        }

    }

    private static Key convertKey(String secret){
...
//        Security.addProvider(new BouncyCastleProvider());
        // 转化成JAVA的密钥格式
        Key key = new SecretKeySpec(keyBytes, AES_ALG);
        return key;
    }
    //    public static byte[] aesEncrypt(byte[] srcBytes, String aesKey){
//        try {
...
//    }
    ...
    public static byte[] aesDecrypt(byte[] targetBytes, String key) {
        try {
            ...
            cipher.init(Cipher.DECRYPT_MODE, /*new SecretKeySpec(base64Decode(key.getBytes()),
                    AES_ALG)*/ convertKey(key),iv);

            return cipher.doFinal(targetBytes);
        } catch (Exception e) {
            e.printStackTrace();
            throw new IllegalArgumentException("AES解密失败");
        }
    }
    ...
    private static final byte[] AES_IV = {0x05, 0x07, 0x06, 0x03, 0x02, 0x0A, 0x03, 0x0B, 0x0A, 0x02, 0x09, 0x05, 0x06, 0x07, 0x06, 0x05};//initIv(AES_CBC_PCK_ALG);
